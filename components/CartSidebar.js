...
function CartGroup({ group, updateQuantity }) {
    ...
    return React.createElement('div', { className: 'border-b dark:border-gray-700' },
        React.createElement('div', { className: 'p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700', onClick: () => setIsExpanded(!isExpanded) },
            React.createElement('div', { className: 'flex justify-between items-center' },
                React.createElement('div', null,
                    React.createElement('p', { className: 'font-bold text-gray-800 dark:text-gray-100' }, group.style),
                    React.createElement('p', { className: 'text-sm text-gray-600 dark:text-gray-400' }, group.color)
                ),
                React.createElement(ChevronIcon)
            ),
             React.createElement('div', { className: 'flex justify-between items-center mt-2 text-sm text-gray-500 dark:text-gray-400' },
                React.createElement('span', null, `Total Qty: ${groupTotalQuantity}`)
            )
        ),
        isExpanded && React.createElement('div', { className: 'bg-white dark:bg-gray-800 px-4 pb-2' },
            React.createElement('div', { className: 'space-y-3' },
                group.variants.map(item =>
                    React.createElement('div', { key: item.barcode, className: 'flex items-center justify-between py-2 border-t dark:border-gray-700' },
                        React.createElement('div', { className: 'font-medium text-gray-700 dark:text-gray-300 w-12' }, item.size),
                        React.createElement('div', { className: 'flex items-center border border-gray-200 dark:border-gray-600 rounded-md' },
                            React.createElement('button', { onClick: () => updateQuantity(item.barcode, item.quantity - 1), className: 'p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-l-md' }, React.createElement(MinusIcon)),
                            React.createElement('span', { className: 'w-10 text-center font-medium text-gray-800 dark:text-gray-200' }, item.quantity),
                            React.createElement('button', { onClick: () => updateQuantity(item.barcode, item.quantity + 1), className: 'p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-r-md' }, React.createElement(PlusIcon))
                        ),
                        React.createElement('div', { className: 'flex items-center space-x-2' },
                            React.createElement('button', { onClick: () => updateQuantity(item.barcode, 0), className: 'p-1 text-red-500 hover:text-red-700 hover:bg-red-50 dark:hover:bg-red-900/50 rounded-full' }, React.createElement(TrashIcon))
                        )
                    )
                )
            )
...
function CartSidebar({ isOpen, onClose, onOrderSuccess }) {
    ...
    return React.createElement('div', { className: `fixed inset-0 z-50 ${isOpen ? 'pointer-events-auto' : 'pointer-events-none'}` },
        ...
        React.createElement('div', { className: `relative w-full max-w-lg ml-auto h-full bg-white dark:bg-gray-800 shadow-xl flex flex-col transition-transform duration-300 ease-in-out transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}` },
            React.createElement('div', { className: 'p-4 border-b dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-800' },
                React.createElement('h2', { className: 'text-xl font-bold text-gray-800 dark:text-gray-100' }, 'Your Cart'),
                React.createElement('button', { onClick: onClose, className: 'text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200' },
            ...
            isLoadingCart ?
                React.createElement('div', { className: 'flex-grow flex flex-col items-center justify-center text-center p-4' },
                    ...
                    React.createElement('p', { className: 'text-gray-500 dark:text-gray-400 mt-4' }, 'Loading Cart...')
                ) :
            cartItems.length === 0 ?
                React.createElement('div', { className: 'flex-grow flex flex-col items-center justify-center text-center p-4' },
                    React.createElement(EmptyCartIcon),
                    React.createElement('p', { className: 'text-gray-500 dark:text-gray-400 mt-4' }, 'Your cart is empty.'),
                    React.createElement('p', { className: 'text-sm text-gray-400 dark:text-gray-500' }, 'Add products to get started.')
                ) :
                React.createElement(React.Fragment, null,
                    React.createElement('div', { className: 'flex-grow overflow-y-auto bg-gray-50 dark:bg-gray-800' },
