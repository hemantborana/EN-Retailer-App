...
function ProductCard({ product, onSelect, isBestSeller }) {
    ...
    return React.createElement('div', {
        ...
        className: 'relative bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col'
    },
        React.createElement('div', { className: 'p-4 flex-grow' },
            React.createElement('div', { className: 'flex justify-between items-start gap-2' },
                 React.createElement('h3', { 
                     className: 'text-base sm:text-lg font-bold text-gray-800 dark:text-gray-100' 
                    }, product.style),
            ...
            React.createElement('p', { 
                className: 'text-xs sm:text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap mt-1' 
            }, priceDisplay)
        ),
        product.colors.length > 0 && React.createElement('div', { className: 'flex items-center p-2 bg-gray-50 dark:bg-gray-700 border-t dark:border-gray-600 space-x-1.5' },
            displayedColors.map(color => {
                ...
                return React.createElement('div', {
                    key: color.code,
                    title: color.name,
                    className: 'h-6 w-6 rounded-md border border-gray-200 dark:border-gray-500 shadow-sm text-[8px] font-bold flex items-center justify-center',
                    style: style
